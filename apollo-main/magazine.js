...
window.toggleMagazineAudio = function(passedTrackUrl) {
    ...
    document.querySelectorAll('.magazine-audio-btn').forEach(btn => {
    ...
    if (isMobile) {
        ...
        if (window._magSpreadView) {
            for (let i = 0; i < leaves; i++) {
                html += '<div class="mag-page-row" style="display: flex; margin: 0 auto 2rem auto; width: 95vw; justify-content: center; scroll-snap-align: center; scroll-snap-stop: always; transition: opacity 0.5s ease;">';
            ...
            mag.forEach(imgUrl => {
                if(imgUrl) html += `<img class="mag-page-row" src="${getSafeImg(imgUrl)}" style="display: block; margin: 0 auto 1rem auto; width: 90vw; height: auto; scroll-snap-align: center; scroll-snap-stop: always; transition: opacity 0.5s ease;">`;
            });
        }
        html += '</div>';
        ...
        for (let i = 0; i < leaves; i++) {
            ...
            let frontAudio = (isTopRight && pages[i*2] && pages[i*2].audioUrl) ? `<div class="magazine-audio-controls"><div class="magazine-audio-btn" onclick="event.stopPropagation(); toggleMagazineAudio('${pages[i*2].audioUrl}')">${magAudioState}</div><div class="magazine-audio-btn" onclick="event.stopPropagation(); navigator.clipboard.writeText('${frontShareUrl}'); this.innerText='copied ↗'; setTimeout(()=>this.innerText='share ↗', 2000)">share ↗</div></div>` : '';
            let backAudio = (isTopLeft && pages[i*2+1] && pages[i*2+1].audioUrl) ? `<div class="magazine-audio-controls"><div class="magazine-audio-btn" onclick="event.stopPropagation(); toggleMagazineAudio('${pages[i*2+1].audioUrl}')">${magAudioState}</div><div class="magazine-audio-btn" onclick="event.stopPropagation(); navigator.clipboard.writeText('${backShareUrl}'); this.innerText='copied ↗'; setTimeout(()=>this.innerText='share ↗', 2000)">share ↗</div></div>` : '';
        ...
        let chaptersHtml = chaptersData.map((ch, idx) => {
            ...
            for (let imgIdx = startImage; imgIdx < endImage; imgIdx++) {
                ...
                if (imgIdx === startImage) {
                    html += `<span class="highlight-link mag-page-item" data-img-idx="${imgIdx}" onclick="event.stopPropagation(); magCurrentPage = ${leafIdx}; updateMagazineView()">${ch.name}</span>`;
                } else {
        ...
        if (!window._magSpineTracker) {
            window._magSpineTracker = function(e) {
                ...
                document.querySelectorAll('.mag-page-item').forEach(el => {
                    if (parseInt(el.dataset.imgIdx) === activeImgIdx) {
