...
async function init() {
    ...
    try {
        ...
        let rawMag = masterData.magazine || [];
        magazineData = Array.isArray(rawMag) ? rawMag : [rawMag];
    ...
    const magazineParam = urlParams.get('magazine');
    ...
    } else if (magazineParam) {
        setTimeout(() => {
            if (isLoaded) {
                let foundIndex = magazineData.findIndex(m => m.id === magazineParam);
                if (foundIndex !== -1) {
                    let startPage = pageParam ? parseInt(pageParam, 10) : 0;
                    if (isNaN(startPage)) startPage = 0;
                    openMagazine(foundIndex, startPage);
                }
            }
        }, 600);
    }
...
function generateDynamicTags() {
    ...
    // Add Magazine tag for 3D issues view as the very last tag
    if (magazineData.length > 0) {
        const magBtn = document.createElement('span');
        magBtn.className = 'tag-filter highlight-link dynamic-tag-item';
        magBtn.style.display = 'none';
        magBtn.innerText = 'magazine';
        magBtn.onclick = (e) => {
            const overlay = document.getElementById('search-overlay');
            if (overlay) {
                overlay.remove();
